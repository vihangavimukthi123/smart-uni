// SupplierEditPackagePage.jsx
import React, { useState, useEffect } from "react";
import { useNavigate, useParams, useLocation, Link } from "react-router-dom";
import api from "../../../api/axios";
import toast from "react-hot-toast";
import { BiSave, BiArrowBack, BiPlus, BiTrash, BiPackage, BiTag, BiMoney, BiAlignLeft, BiRefresh, BiCollection, BiCheckShield } from "react-icons/bi";
...
export default function SupplierEditPackagePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (location.state) {
          ...
          const res = await api.get(`/rental/packages/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await api.put(`/rental/packages/${id}`, formData);
  ...
  return (
    <div className="page-wrapper pb-20 anim-fadeIn">
      ...
      <div style={{ marginBottom: '48px' }}>
        ...
        <h1 style={{ fontSize: '36px', fontWeight: '900', color: textPrimary, letterSpacing: '-0.02em', marginBottom: '8px' }}>Edit Rental Bundle</h1>
        <p style={{ color: textSecondary, fontSize: '12px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
          Syncing specifications for <span style={{ color: '#6366f1' }}>#{id}</span>
        </p>
      </div>
